...
1. Environment Setup
Clone the repository and install all required system packages:

git clone [https://github.com/HarsheetT/SymphonySynth.git](https://github.com/HarsheetT/SymphonySynth.git)
cd SymphonySynth
pip install -r requirements.txt

2. Run Data Tokenization
To process raw symbolic MIDI tracks into sequential token structures matching the input bounds:

python src/tokenizer.py --input_dir data/raw_midi/ --output_dir data/tokenized/

3. Training & Fine-Tuning Execution
You can execute both pre-training and domain-specific fine-tuning phases directly from the pipeline:

# Phase 1: Base Pre-training on Western Dataset
python src/train.py --mode base --epochs 10

# Phase 2: Targeted Domain Fine-Tuning on Indian Raga Dataset
python src/train.py --mode finetune --epochs 3

4. Deploy the Local Web App
Launch the responsive UI to test real-time interactive generation locally:
